...
export const feeAccount = new PublicKey(
  '2xhBxVVuXkdq2MRKerE9mr2s1szfHSedy21MVqf8gPoM'  //set to address of project owner
);
...
export class GemFarmClient extends GemBankClient {
  ...
  constructor(
    conn: Connection,
    wallet: typeof Wallet, //anchor.Wallet
    farmIdl?: Idl,
  ...
  setFarmProgram(idl?: Idl, programId?: PublicKey) {
    ...
    if (idl && programId) {
      ...
      //console.log("setFarmProgram-provider",this.provider)
  ...
  async fetchFarmAcc(farm: PublicKey) {
    console.log("fetchFarmAccfetchFarmAcc",farm.toBase58())
  ...
  async initFarmer(
    ...
    console.log("initFarmer", farm)
    const farmAcc = await this.fetchFarmAcc(farm);

    const [farmer, farmerBump] = await this.findFarmerPDA(farm, identityPk);
    const [vault, vaultBump] = await this.findVaultPDA(farmAcc.bank, identityPk);
    ...
    console.log("farmProgram", this.farmProgram.rpc);
    console.log("farmerBump", farmerBump);
    console.log("vaultBump", vaultBump);
    console.log("farm", farm);
    console.log("farmer", farmer);
    console.log("identityPk", identityPk);
    console.log("farmAcc.bank", farmAcc.bank);
    console.log("vault", vault);
    console.log("bankProgram", this.bankProgram.programId);
    console.log("payer", isKp(payer) ? (<Keypair>payer).publicKey : payer);
    console.log("signers", signers); */
    //console.log("fee feeAccount", feeAccount.toBase58());
    //feeAcc: feeAccount, //using the root gem_farm.json
  ...
  async stakeCommon(
    ...
    //console.log("stakeCommon",farm.toBase58())
    const farmAcc = await this.fetchFarmAcc(farm);
    ...
    if (unstake) {
      ...
      try {
          //console.log("farmProgramRPC123", this.farmProgram.rpc)
          txSig = await this.farmProgram.rpc.unstake(farmAuthBump,
          farmTreasuryBump,
          farmerBump,
          farm.toBase58(), {
              accounts: {
          ...
          console.log('UNstaking Error: ', err);
       }
    } else {
