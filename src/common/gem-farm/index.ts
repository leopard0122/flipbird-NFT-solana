...
//initGemFarm(conn: Connection, wallet?: SignerWalletAdapter)
export async function initGemFarm(conn: Connection, wallet?: Wallet) {
  ...
  return new GemFarm(
    conn,
    walletToUse as any, // walletToUse as typeof anchor.Wallet,
    farmIdl,
    bankIdl
  );
}
...
export class GemFarm extends GemFarmClient {
  constructor(
    ...
    // @ts-ignore
    this.wallet = wallet.adapter; //added the .adapter here too
  }
  ...
  async unstakeWallet(farm: PublicKey) {
    console.log("try unstakeWallet for farmer", farm);
    // @ts-ignore
